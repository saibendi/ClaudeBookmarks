...
### #10 — Missing-audio videos: fetch music.playUrl as a fallback instead of accepting the loss

**Question:** Some videos (2 of 28 fetched so far, ~7%) downloaded with a video stream but zero audio streams — TikTok serves them only as DASH-split components with no fetchable combined stream (confirmed: the separate audio DASH URL and the alternate combined-stream URL both `403 Access Denied` from TikTok's CDN — those signed URLs need session/cookie auth only a downloader's own HTTP client provides; `yt-dlp` picked the byte-identical video-only asset, so it's not a "switch tools" fix either). Originally logged as an accepted, unfixed limitation. Is there actually a way around it?

**Decision:** Yes — `music.playUrl` in the metadata (TikTok's reusable "sound" object, a different subsystem from the raw video CDN) is fetchable with a plain request where the video-CDN URLs 403. `gallery-dl` already proves this works: its `_extract_audio()` fetches this exact field successfully today, just gated to photo posts only (`if "imagePost" in post`), never wired up for video posts missing audio. Verified manually end-to-end before building it in: fetched `music.playUrl` for `@sedoxo`'s video cleanly (no CDN block), found its duration matched the video to within 16ms (this was an "original sound" post — the video is 431s/7+ minutes long, and `music.playUrl` turned out to just be that same video's own audio track in full, not a separate segment), muxed with `ffmpeg` (`-c:v copy -c:a aac -shortest`), confirmed real non-silent audio via `volumedetect` (-31.9dB mean, not silence).

For posts using a trending/licensed song instead of "original sound," `music.duration` will typically be longer than the video with no offset field in the metadata to know which segment plays — `-shortest` still trims to the right length, just not guaranteed to start at the exact right point in the song. Real audio either way, imperfect sync only in that one sub-case.

Added `has_audio_stream()`/`fetch_music_track()`/`mux_audio()`/`ensure_audio()` to `fetch_tiktoks.py`, same shape as the existing `ensure_h264()` — `ffprobe` check → conditional fetch → `ffmpeg` fix, wired into the main fetch loop right after `ensure_h264` (so the mux step can safely `-c:v copy` an already-H.264 video). Extended `--reencode-existing`'s maintenance sweep to also check/fix audio, loading each video's sibling `.json` for the `music` field. Ran it against the 2 known-broken videos: both fixed, verified `h264,video` + `aac,audio` streams present and real non-silent audio levels on both, 20 other already-fine videos correctly left untouched, no leftover temp files.

## Known Limitations

None currently — the DASH-split missing-audio issue was found and fixed (see Decision #10), not left open. New entries go here if something similar turns up without a clean fix.
